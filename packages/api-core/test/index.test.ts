...
describe('APICore', () => {
  ...
  beforeEach(async () => {
    fileUploads = await loadSpec('@readme/oas-examples/3.0/json/file-uploads.json').then(
    ...
    petstore = await loadSpec('@readme/oas-examples/3.0/json/petstore-expanded.json').then(
      spec => new APICore(spec),
    )

    readme = await loadSpec('@readme/oas-examples/3.0/json/readme.json').then(
      spec => new APICore(spec),
    )

    security = await loadSpec('@readme/oas-examples/3.0/json/security.json').then(
      spec => new APICore(spec),
    )
  ...
  describe('#fetch', () => {
    ...
    describe('payload delivery', () => {
      it('should pass through body for method + path', async () => {
        ...
        await petstore
          .fetch('/pets', 'post', body)
          .then(({data}) => expect(data).toStrictEqual(body))
      })

      it('should pass through parameters for method + path', async () => {
        const slug = 'new-release'
        fetchMock.put(
          `https://dash.readme.com/api/v1/changelogs/${slug}`,
      ...
      describe('application/x-www-form-urlencoded', () => {
        it('should support `application/x-www-form-urlencoded` requests', async () => {
          ...
          const {data} = await usptoSpec.fetch(
      ...
      describe('multipart/form-data', () => {
        ...
        describe('files', () => {
          it('should support plaintext files', async () => {
            fetchMock.post(
              'https://httpbin.org/anything/multipart-formdata',
              mockResponse.multipart,
            )
  ...
  describe('#setServer()', () => {
    it('should support supplying a full server url', async () => {
      ...
      await serverVariables
        .fetch('/', 'post')
        .then(({data}) => expect(data).toStrictEqual(response))
    })
  ...
  describe('#setConfig()', () => {
    describe('timeout', () => {
      ...
      beforeEach(async () => {
        petstoreTimeout = await loadSpec('@readme/oas-examples/3.0/json/petstore.json').then(
          spec => new APICore(spec),
        )
      })
      ...
      it('should override the default `fetch` timeout if present and fail if request takes too long', async () => {
        fetchMock.delete(
          ...
          mockResponse.delay(response, 500),
        )

        petstoreTimeout.setConfig({timeout: 100})

        await expect(petstoreTimeout.fetch(`/pet/${petId}`, 'delete')).rejects.toThrow(
      ...
      it('should override the default `fetch` timeout and return if request is quick', async () => {
        fetchMock.delete(
          `http://petstore.swagger.io/v2/pet/${petId}`,
          mockResponse.delay(response, 100),
        )
