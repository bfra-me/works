...
describe('#prepareParams', () => {
      ...
      it('should handle a multipart body when a property is a file stream', async () => {
        ...
        const body = {
          documentFile: fs.createReadStream(
            require.resolve('@api/test-utils/fixtures/owlbert.png'),
          ),
        }
  ...
  describe('parameter types', () => {
    ...
    beforeEach(async () => {
      parameterStyle = await loadSpec('@readme/oas-examples/3.1/json/parameters-style.json').then(
        Oas.init,
      )
      ...
      it('should not send special headers in body payloads', async () => {
        ...
        await expect(
          prepareParams(
            operation,
            {scope: 'SERVER_ACCESS'},
            {accept: 'application/json', 'BASIQ-version': '3.0'},
          ),
    ...
    it('should support path parameters', async () => {
      const operation = parameterStyle.operation(
